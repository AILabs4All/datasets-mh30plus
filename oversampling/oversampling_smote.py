...
def validation(dataset_path):
    ...
    try:
        samples_file = pd.read_csv(dataset_path)
...
if __name__ == "__main__":
    ...
    dataset_file_loaded = pd.read_csv(arguments.input_dataset)
    ...
    # Reorder columns using copy() to avoid fragmentation
 
    print(f"Resampled shape: {resampled_df.shape}")
        
    aux = arguments.input_dataset.split(".")
    new_file_name = aux[0] + "_oversampling.csv"
    resampled_df.to_csv(new_file_name, index=False, sep=',', header=True) 
 
    
    print(f"Oversampled dataset saved as: {new_file_name}")
